...
export function drawGlobalNoise(ctx, width, height, mode) {
  const alpha = mode === "easy" ? 0.018 : mode === "medium" ? 0.028 : 0.034;
...
function drawCollapseAmbient(ctx, width, height, now) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const x = (baseX + noiseA * sway + noiseB * sway * 0.35 - 18) % (width + 36);
...
export function drawHardEffects(ctx, width, height, theme, now) {
  const vignette = ctx.createRadialGradient(
    width / 2,
    height / 2,
